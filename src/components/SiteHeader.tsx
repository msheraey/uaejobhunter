...
export function Logo({ className = "" }: { className?: string }) {
  return (
    <Link to="/" className={`flex items-center gap-2 font-bold text-xl ${className}`}>
      ...
      <span className="tracking-tight">Job<span className="text-gradient-brand">Hunter</span></span>
    </Link>
  );
}
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <>
              ...
              <Button variant="outline" onClick={async () => { await signOut(); router.navigate({ to: "/" }); }}>Sign out</Button>
            </>
          ) : (
            <>
              <Button variant="ghost" asChild className="hidden sm:inline-flex"><Link to="/login">Login</Link></Button>
              <Button asChild className="bg-gradient-brand hover:opacity-90 shadow-elegant text-primary-foreground">
